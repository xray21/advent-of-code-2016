...
const getChecksum = str => {
	...
	chars.forEach(c => {
		var curObj = counts.find(countObj => countObj.char == c);
	...
	counts = counts.sort( (a, b) => a.count - b.count || b.char.charCodeAt(0) - a.char.charCodeAt(0) ).reverse();
	
	return counts.slice(0, 5).map(c => c.char).join("");
}

const organizeStr = str => str.split("").sort((a,b) => a.charCodeAt(0) - b.charCodeAt(0)).join("");

const shiftStr = (str, n) => {
	return str.toUpperCase().split("").map(c => {
		return c == " " ? " " : String.fromCharCode(((c.charCodeAt(0) - 65 + (n % 26)) % 26) + 65);
	}).join("");
}
...
const part1 = () => {
	...
	input.forEach(str => {
		var parts = str.split("-");
		var encryptedName = parts.slice(0, parts.length - 1).join("");
		var sectorIDChunks = parts[parts.length - 1].split("[");
		var sectorID = parseInt(sectorIDChunks[0]);
		var checksum = sectorIDChunks[1].replace("]", "");
		var compareChecksum = getChecksum(encryptedName);
		
		sectorIDSum += (checksum == compareChecksum ? sectorID : 0);
	})
	...
	console.log(
		input.map(str => {
			...
			return {
				...
				sectorID,
				checksum,
				compareChecksum: getChecksum(encryptedName.replace(/ /g, ""))
			}
		})
		.filter(obj => obj.checksum == obj.compareChecksum)
		.map(obj => {
			return {
				realName: shiftStr(obj.encryptedName, obj.sectorID), 
				sectorID: obj.sectorID
			}
		})
		.sort((a,b) => a.realName < b.realName ? -1 : 1)
		.map(obj => obj.sectorID + " - " + obj.realName)
	)
}; 
